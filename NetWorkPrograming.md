...
- LT模式编程更符合用户直觉，业务层逻辑简单，不易出错，但效率比ET低
- ET模式更加高效，但也容易遗漏事件，容易产生bug
- nginx使用的是ET模式从而达到并发高性能。Redis使用的是LT，可以减少使用过程中的bug
- 当并发量小时建议使用LT，读写逻辑简单，不易遗漏事件。在并发量大时应使用ET以提高效率
