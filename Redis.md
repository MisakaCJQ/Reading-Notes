...
某一个热点key，在失效的一瞬间被大量请求，然后这些请求都会打向数据库，造成数据库压力骤增甚至宕机。

解决方案：

- 可使用互斥锁。在并发访问时加锁，使得第一个线程请求后从数据库取到数据放入缓存。然后其他的线程直接访问缓存
- 将热点数据设置为不过期
...
大量的热点key在同一时间失效，使得缓存在同一时刻全部失效。造成数据库瞬时访问量过大，甚至宕机。

解决方案：

- 可以在每个key过期时间的基础上加上一个随机值，使其不会同时过期。
- 让热点数据不要过期
...
有时用户会访问一个缓存和数据库中都没有的key，此时就会直接打到数据库上，且查不到数据无法写缓存。

解决方案：

- 如果在数据库中找不到key，则可以缓存一个key对应的空值，并设置较短的过期时间
- 使用布隆过滤器。布隆过滤器可以判定一定不存在或可能存在两种，从而对这种。

布隆过滤器在写入key时使用多组哈希函数对其哈希，然后将这些位都置为1。

在查询时同样进行哈希，如果哈希到的位都为1则判定为可能存在，如果至少有一个不为1则判定key一定不存在